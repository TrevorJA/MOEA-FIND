...
def resample_disjoint_tilings(
    monthly_1d: np.ndarray,
    T_years: int,
    n_offsets: int = None,
) -> List[List[np.ndarray]]:
    """Multiple disjoint stride-T tilings of a historical record.

    Each tiling is a sequence of non-overlapping T-year blocks. Within
    one tiling, no water year appears twice — so per-tiling correlation
    estimates between block-level metrics are unbiased by shared-year
    autocorrelation. By default returns ``T_years`` distinct tilings
    (start offsets 0, 1, ..., T_years − 1) so every starting alignment
    of the disjoint grid is represented.

    For N_hist = 73, T = 5: each tiling contains 14 blocks (the last
    1-3 years are dropped); 5 tilings → 70 block-metric observations
    in total, with each year appearing exactly 5 times across the
    union but only once within any single tiling.

    Args:
        monthly_1d: 1D historical monthly flows, water-year-aligned.
        T_years: Block length in water years.
        n_offsets: Number of distinct start offsets (default ``T_years``,
            for maximum coverage).

    Returns:
        List of tilings; each tiling is a list of 1D arrays of length
        ``T_years * 12``.
    """
    x = np.asarray(monthly_1d, dtype=float)
    if x.ndim != 1:
    ...
    n_hist_months = len(x)
    n_hist_years = n_hist_months // 12
    if n_hist_years < T_years:
        raise ValueError(
            f"historical record has {n_hist_years} water years, "
            f"need at least T_years={T_years}"
        )
    if n_offsets is None:
        n_offsets = T_years

    months_per_block = T_years * 12
    tilings: List[List[np.ndarray]] = []
    for off in range(n_offsets):
        start = off * 12
        blocks: List[np.ndarray] = []
        while start + months_per_block <= n_hist_months:
            blocks.append(x[start : start + months_per_block].copy())
            start += months_per_block
        if blocks:
            tilings.append(blocks)
    return tilings
...
def compute_historical_block_chars(
    ...
    Args:
        ...
        ssi_calc: Prefitted SynHydro SSI calculator (see
            :func:`src.metrics.objectives.make_ssi_calculator` +
            :func:`src.metrics.objectives.compute_ssi` with
