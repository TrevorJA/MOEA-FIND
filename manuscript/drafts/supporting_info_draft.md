...
## SI-4b. Constraint regime ablation — DV-uniformity vs. hydrologic constraints

*Cross-referenced from main text section 3.3 (constraint design) and design_decisions.md §DD-14.*

The main-text production run uses a single Anderson-Darling goodness-of-fit statistic applied to the decision-variable vector as the plausibility constraint (hereafter DV-AD). This section documents the empirical basis for that choice by comparing three constraint regimes across three random seeds at 200,000 function evaluations each on the Cannonsville single-site problem.

**Constraint regimes compared:**

- *Hydrologic (5-statistic):* annual mean, annual CV, lag-1 autocorrelation, non-drought mean flow, and seasonal cycle, each calibrated by a bootstrap tolerance against the historical Cannonsville record.
- *DV L2-star discrepancy:* a single constraint measuring the L2-star discrepancy of the 240-element decision-variable vector from uniformity.
- *DV Anderson-Darling (production):* a single Anderson-Darling statistic on the 240-element decision-variable vector, calibrated by bootstrap uniform draws at the 95th-percentile significance level.

**Key results** (exp13/14 ablation; all three seeds pooled):

| Regime | Pareto size | Max duration (mo) | Max severity | Max peak month | Manhattan median |
|---|---|---|---|---|---|
| Hydrologic | 4662 | 9.50 | 2.70 | 4.86 | 72.15 |
| DV L2-star | 5602 | 9.47 | 2.85 | **12.00** | 72.57 |
| DV AD (production) | 3939 | 8.71 | 2.50 | 5.00 | 72.36 |

The DV L2-star regime tolerates a `peak_severity_month` value of 12.0, which saturates the cyclic metric's upper bound and is a numerical artefact of the DV vector exploring the boundary of the unit hypercube unconstrained by any statistic sensitive to tail behaviour. The Anderson-Darling statistic is more sensitive to distributional tails than the L2-star discrepancy and eliminates this artefact while preserving comparable drought-space coverage (Manhattan-distance median within 0.21 of the hydrologic baseline).

**Justification for the AD constraint as the production choice** is given in full in design_decisions.md §DD-14. In brief: the AD constraint provides a single, calibration-traceable threshold with a clear operational definition, produces a physically defensible Pareto front with no boundary artefacts, and achieves coverage equivalent to the more complex hydrologic formulation at identical NFE budgets.

The seven SI figures for this ablation (figSI\_ablation\_pareto\_2d.pdf through figSI\_ablation\_dv\_tail\_mass.pdf) are generated by `workflows/experiments/14_dv_uniformity_compare.py` from `outputs/exp13_dv_uniformity_ablation/`.
