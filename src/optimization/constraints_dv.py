...
Alternative to the five hydrologic-space constraints in :mod:`src.optimization.constraints`.
...
:mod:`src.optimization.constraints` so that downstream code (``run_experiment``, diagnostic
...
from src.optimization.constraints import ConstraintResult, Violation, _two_tier
...
def compute_dv_constraint(
    ...
    Returns a :class:`src.optimization.constraints.ConstraintResult` with a single-element
    ``hard_violations`` list, matching the shape expected by
    :func:`src.experiment.run_experiment` when ``n_constrs = 1``.
