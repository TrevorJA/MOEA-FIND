...
def prep_predicted_inflows(
    ...
    # Step 0: pre-simulate STARFIT releases. pywrdrb's
    # PredictedInflowEnsemblePreprocessor in perfect_foresight mode now
    # *requires* presimulated_releases_mgd.hdf5 (written into the flow-type
    # dir by STARFITReleaseEnsemblePreprocessor); without it predict_inflows
    # raises FileNotFoundError. This dependency post-dates the OLD-baseline
    # run, so it only surfaces when the pipeline is re-run for a fresh slug.
    from pywrdrb.pre import STARFITReleaseEnsemblePreprocessor

    if rank == 0:
        print(f"[bridge] Step 0/{n_steps + 1}: STARFITReleaseEnsemblePreprocessor "
              f"(presimulated releases) for {len(realization_ids)} realizations "
              f"({'MPI' if use_mpi else 'serial'})")
    starfit_pre = STARFITReleaseEnsemblePreprocessor(
        inflow_type=flow_type,
        realization_ids=realization_ids,
        use_mpi=use_mpi, comm=comm,
        # force=True: always (re)build presimulated_releases_mgd.hdf5.
        # Default force=False treats any existing file as a valid cache and
        # skips regeneration — which silently preserved the corrupt empty
        # file left by the earlier HDF5-attr failure, so PredictedInflow
        # then failed with "Missing reservoirs". Re-evals must regenerate.
        force=True,
    )
    starfit_pre.run()
    del starfit_pre
    if use_mpi and comm is not None:
        # Ensure presimulated_releases_mgd.hdf5 is fully written by all
        # ranks before PredictedInflow ranks open it for reading.
        comm.Barrier()
