...
# src/experiment/config.py -> parents[2] == MOEA-FIND root.
...
class ExperimentConfig:
    ...
    :data:`src.metrics.drought_metrics.PRESETS` (``"primary"``,
    ``"extreme_event"``, ``"trace_fdc"``, ``"legacy"``) or any single
    metric name from :data:`src.metrics.drought_metrics.REGISTRY`. Per-axis
    ...
    metric live on the :class:`src.metrics.drought_metrics.DroughtMetric`
    ...
    metric_set:
        ...
        :mod:`src.metrics.drought_metrics`. Default ``"primary"``: mean event
