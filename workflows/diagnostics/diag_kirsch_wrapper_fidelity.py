...
Run (SLURM only — not on the login node):
    sbatch workflows/slurm/diag_kirsch_wrapper_fidelity.slurm
"""
...
def _build_kirsch_generator(monthly_2d: np.ndarray):
    ...
    Mirrors the helper in workflows/experiments/05_kirsch_library_build.py
    and workflows/experiments/13_dv_uniformity_ablation.py. Kept local so
    the diagnostic has no cross-dependency on experiment drivers.
    """
    import pandas as pd
