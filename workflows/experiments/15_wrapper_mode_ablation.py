...
Run locally (small NFE smoke test):
    python workflows/experiments/15_wrapper_mode_ablation.py \\
        --wrapper-mode index --nfe 200 --seed 42 --algorithm eps_nsga2

Run on HPC (MM Borg, full NFE, array over modes x seeds):
    sbatch workflows/slurm/15_wrapper_mode_ablation.slurm
"""
...
def _load_dv_constraint(
    ...
    if json_path is None:
        print("[15] WARNING: no DV-uniformity JSON supplied; "
              "running unconstrained.")
        return None
    json_path = Path(json_path)
    if not json_path.exists():
        print(f"[15] WARNING: DV-uniformity JSON not found at {json_path}; "
    ...
    print(f"[15] DV-uniformity constraint loaded from {json_path} "
...
def main():
    ...
    dv_cfg: DVUniformityConfig | None = _load_dv_constraint(
    ...
    # for the SI are produced by workflows/experiments/16_wrapper_mode_compare.py.
