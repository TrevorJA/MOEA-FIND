...
Run locally (small NFE smoke test):
    python workflows/experiments/13_dv_uniformity_ablation.py \\
        --arm dv_uniform --nfe 200 --seed 42 --algorithm eps_nsga2

Run on HPC (MM Borg, full NFE, array over arms x seeds):
    sbatch workflows/slurm/13_dv_uniformity_ablation.slurm
"""
...
def _build_kirsch_generator(monthly_2d: np.ndarray):
    ...
    Mirrors the helper in ``04_kirsch_single_site.py`` — kept local so the
    ablation driver stays self-contained and the main-text script is
    untouched.
    """
    import pandas as pd
...
def _load_hydrologic_constraints(
    ...
    if json_path is None:
        print("[13] WARNING: hydrologic arm selected but no constraints JSON "
              "supplied; running unconstrained.")
        return None
    json_path = Path(json_path)
    if not json_path.exists():
        print(f"[13] WARNING: hydrologic constraints JSON not found at "
    ...
    print(f"[13] hydrologic constraints loaded from {json_path}")
    return cfg
...
def _load_dv_constraint(
    ...
    if json_path is None:
        print("[13] WARNING: dv_uniform arm selected but no DV-uniformity JSON "
              "supplied; running unconstrained.")
        return None
    json_path = Path(json_path)
    if not json_path.exists():
        print(f"[13] WARNING: DV-uniformity JSON not found at {json_path}; "
    ...
    print(f"[13] DV-uniformity constraint loaded from {json_path} "
          f"(statistic={cfg.statistic}, tol={cfg.tolerance:.4g}, "
          f"n_dvs={cfg.n_dvs})")
    return cfg
...
def main():
    ...
    if args.arm == "hydrologic":
        hydrologic_cfg = _load_hydrologic_constraints(
            args.constraints_json, args.site_label, args.n_years,
        )
    else:  # dv_uniform
        dv_cfg = _load_dv_constraint(
    ...
    # for the SI are produced by workflows/experiments/14_dv_uniformity_compare.py.
